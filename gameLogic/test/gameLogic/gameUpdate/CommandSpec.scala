...
class CommandSpec extends FunSuite with Matchers with TestData {
  ...
  test("RegisterForGame: add players") {
    val after1 = RegisterForGame(p1).apply(starting).newState
    after1 shouldBe withPlayer1
    val after2 = RegisterForGame(p2).apply(after1).newState
    after2 shouldBe withPlayer1And2
  }

  test("RegisterForGame: reject players with the same name") {
    RegisterForGame(p1).apply(withPlayer1).rejectedReason shouldBe PlayerAlreadyRegistered
  }
  ...
  test("ReadyForGame: start game if all players are ready") {
    val after1 = ReadyForGame(p1).apply(withPlayer1And2).newState
    after1 shouldBe withPlayer1Ready
    val after2 = ReadyForGame(p2).apply(withPlayer1Ready).newState
